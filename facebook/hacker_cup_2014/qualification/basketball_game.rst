...
lower shot percentage. If two players have the same shot percentage, the taller
player is rated higher.
...
where the highest-rated player gets the number 1, the second highest-rated gets
the number 2, and so on. Now the first team contains all the players with the
odd draft numbers and the second team all the players with the even draft
numbers.
...
If there are more than **P** players on a team after each minute of the game
the player with the highest total time played leaves the playing field. Ties
are broken by the player with the higher draft number leaving first.
...
names of all the players currently on the field, (that is after **M**
rotations).
...
by 2 * **P** space separated names of the players playing after **M**
...
`View online <https://www.facebook.com/hackercup/problems.php?pid=740733162607577&round=598486203541358>`_.
