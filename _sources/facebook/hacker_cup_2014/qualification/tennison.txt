...
Tennison wins a set with probability p\ :sub:`s` when it's sunny, and with
probability p\ :sub:`r` when it's raining. The chance that there will be sun
for the first set is p\ :sub:`i`. Luckily for Tennison, whenever he wins a
set, the probability that there will be sun increases by p\ :sub:`u` with
probability p\ :sub:`w`. Unfortunately, when Tennison loses a set, the
probability of sun decreases by p\ :sub:`d` with probability p\ :sub:`l`.
What is the chance that Tennison will be successful in his match?
...
the probabilities p\ :sub:`s`, p\ :sub:`r`, p\ :sub:`i`, p\ :sub:`u`,\ p :sub:`w`,
p\ :sub:`d`, p\ :sub:`l` in that order. All of these values are given with
exactly three places after the decimal point.
...
guaranteed that the output is unaffected by deviations as large as 10 :sup:`-8`.
...
0 ≤ p\ :sub:`s`, p\ :sub:`r`, p\ :sub:`i`, p\ :sub:`u`, p\ :sub:`w`, p\ :sub:`d`,
p\ :sub:`l` ≤ 1
...
`View online <https://www.facebook.com/hackercup/problems.php?pid=373965339404375&round=598486203541358>`_.
